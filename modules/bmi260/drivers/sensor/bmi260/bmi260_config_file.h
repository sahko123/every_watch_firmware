 ...
 * Copyright (c) 2026 Every Watch
 *
 * SPDX-License-Identifier: BSD-3-Clause
 *
 * BMI260 feature-engine configuration blob.
 *
 * This is microcode for the sensor's internal feature engine, uploaded into
 * its RAM over I2C on every power-up (it is volatile — nothing is programmed
 * permanently). The driver never interprets it: it is written verbatim to
 * INIT_DATA, after which INTERNAL_STATUS must report INIT_OK. It supplies the
 * implementations of the motion features; which of them are enabled, and with
 * what thresholds, is controlled separately through the paged feature
 * registers — see bmi260_trigger.c. Bosch does not publish its source, and no
 * toolchain for the engine's instruction set exists publicly.
 *
 * IMPORTANT: this is NOT interchangeable with the BMI270 blob. Both are 8192
 * bytes and share a 6-byte prologue, but 5662 of 8192 bytes differ — they are
 * different programs for different feature sets. At least one third-party
 * "BMI260" driver in the wild actually ships the BMI270 blob under a BMI260
 * name, so verify by hash rather than by filename.
 *
...
#define ZEPHYR_DRIVERS_SENSOR_BMI260_CONFIG_FILE_H_

#include <zephyr/types.h>

static const uint8_t bmi260_config_file[] = {
...
#endif /* ZEPHYR_DRIVERS_SENSOR_BMI260_CONFIG_FILE_H_ */
