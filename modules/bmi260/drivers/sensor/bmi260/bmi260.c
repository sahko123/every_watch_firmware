...
static const struct sensor_driver_api bmi260_driver_api = {
...
/*
 * One config blob, unlike the BMI270 driver this was forked from — that one
 * ships two (an 8 KB "base" with the full feature set, and a 328 B
 * "max_fifo" that trades every feature for FIFO depth) and picks between
 * them by devicetree compatible. Only the equivalent of "base" has been
 * located for the BMI260, so there is nothing to choose between and the
 * selection machinery is dropped rather than left as a one-armed switch.
 *
 * The anymo_1/anymo_2 feature-register addresses are inherited unchanged
 * from the BMI270. They are UNVERIFIED on this part: the register file is
 * laid out by the microcode, and the BMI260's blob differs from the
 * BMI270's in 5662 of 8192 bytes, so any-motion may live elsewhere or not
 * be implemented at all. Bosch treats the per-part feature list as NDA
 * material, so this has to be settled empirically — see the capability
 * readback in the project's IMU bring-up notes. If any-motion turns out to
 * be absent, SENSOR_TRIG_MOTION support here should be removed rather than
 * left silently writing into whatever those addresses actually are.
 */
static const struct bmi260_feature_config bmi260_feature_base = {
	.name = "base",
	.config_file = bmi260_config_file,
	.config_file_len = sizeof(bmi260_config_file),
