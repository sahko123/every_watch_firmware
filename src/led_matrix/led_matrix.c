 ...
 *   T1H = 2×500 = 1000 ns  (spec max 950 ns — marginal, verify on scope)
 *   T1L = 1×500 =  500 ns  (spec 300–600 ns ✓)
 *   T0H = 1×500 =  500 ns  (spec 250–550 ns ✓)
...
void led_commit(void)
{
	...
	static const struct spi_buf tx1_buf = {.buf = buf01, .len = sizeof(buf01)};
	static const struct spi_buf tx2_buf = {.buf = buf23, .len = sizeof(buf23)};
	static const struct spi_buf tx3_buf = {.buf = buf45, .len = sizeof(buf45)};
	static const struct spi_buf_set tx1 = {.buffers = &tx1_buf, .count = 1};
	static const struct spi_buf_set tx2 = {.buffers = &tx2_buf, .count = 1};
	static const struct spi_buf_set tx3 = {.buffers = &tx3_buf, .count = 1};
